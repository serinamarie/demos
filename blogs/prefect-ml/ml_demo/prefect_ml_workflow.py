...
@task(name='get-models', description='Retrieve models with hyperparams')
...
def my_first_ml_flow(n_estimators=200):
    with tags("dev"):
        X_train, X_test, y_train, y_test = create_data().result()
        models = get_models(n_estimators).result()
        training_runs = [train_model(model, X_train, X_test, y_train, y_test) for model in models]
        get_results(training_runs)

# ~/.prefect/storage contains reference to our flow code and other metadata
DeploymentSpec(
    flow=my_first_ml_flow,
